...
public class PIDI_PlanarReflection:MonoBehaviour{
	...
	//INTERNAL
	private RenderTexture v_oldRend; //Buffered static render texture (to detect changes / loss of the static output )
	private RenderTexture v_oldDepth; //Reference to the depth texture used in the previous step so we can release it from memory
...
public void OnWillRenderObject(){
		...
		if ( transform.InverseTransformPoint( v_srcCamera.transform.position ).z < 0 && !b_useExplicitNormal ){
			...
			v_surfaceNormal = b_useExplicitNormal?(b_localNormal?transform.TransformDirection(v_explicitNormal):v_explicitNormal):transform.forward;//We use the forward direction as our normal.
		...
		if (  v_oldDepth && v_oldDepth.IsCreated() ){
...
public void OnWillRenderObject(Camera withCamera){
	...
	public void OnDrawGizmos(){
		...
		if ( b_displayGizmos ){
			Gizmos.matrix = Matrix4x4.TRS ( transform.position, b_useExplicitNormal?Quaternion.FromToRotation(transform.up,(b_localNormal?transform.TransformDirection(v_explicitNormal):v_explicitNormal)):transform.rotation, Vector3.one );
			Gizmos.color = new Color ( 0,0,1, 0.35f );
			Gizmos.DrawCube( Vector3.zero, new Vector3( v_mirrorSize.x, v_mirrorSize.y, 0.08f ) );
			Gizmos.color = Color.cyan;
			Gizmos.matrix = Matrix4x4.identity;
			Gizmos.DrawRay( new Ray( transform.position, b_useExplicitNormal?(b_localNormal?transform.TransformDirection(v_explicitNormal):v_explicitNormal):transform.forward ) );
		}
