...
namespace DXFConvert
{
    ...
    public class POINT : ENTITIE
    {
        public POINT() { }

        //100	子类标记 (AcDbText)
        //39	厚度（可选；默认值 = 0）
        public string C100 { get; set; }//子类标记 (AcDbPoint) 
        ...
        public double C39 { get; set; } //39	厚度（可选；默认值=0） 
        public string C210 { get; set; } //210	拉伸方向（可选；默认值=0,0,1）DXF：X值；APP：三维矢量
        public double C220 { get; set; }//220
        ...
        public POINT(ILoader dxfData, Property prop)
            : base(dxfData, prop)
        {
        ...
        protected override bool ReadProperty(Property prop)
        {
            switch (prop.Code)
            {
                ...
                case 210: C210 = prop.Value; break;
