...
public class FreeCamera : MonoBehaviour {
    ...
    public float zoomSensitive = 0.2f;
    public float nowZoom = 0f,perZoom = 0f;
    public Vector2 ZoomLimite;
    public void ChargeZoom() {
        nowZoom += Input.GetAxis("Mouse ScrollWheel") * zoomSensitive;
        nowZoom = nowZoom > ZoomLimite.y ? ZoomLimite.y : nowZoom;
        nowZoom = nowZoom < ZoomLimite.x ? ZoomLimite.x : nowZoom;
        tourCamera.localPosition += tourCamera.forward.normalized*(nowZoom - perZoom);
        perZoom = nowZoom;
    }
